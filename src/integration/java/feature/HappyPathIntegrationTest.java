...
// 2. when I post to /songs with Song "Till i collapse" then Song "Til i collapse" is returned with id 1
        mockMvc.perform(post("/songs")
                        .content("""
                                {
                                    "name": "Til i collapse",
                                    "releaseDate": "2026-03-11T17:19:52.046Z",
                                    "duration": 0,
                                    "language": "ENGLISH"
                                }
                                """.trim())
                ...
                .andExpect(jsonPath("$.song.name", is("Til i collapse")))
...
// 3. when I post to /songs with Song "Lose Yourself" then Song "Lose Yourself" is returned with id 2
        mockMvc.perform(post("/songs")
                        .content("""
                                {
                                  "name": "Lose Yourself",
                                  "releaseDate": "2026-03-12T16:29:53.464Z",
                                  "duration": 0,
                                  "language": "ENGLISH"
                                }
                                """.trim())
...
// 10. when I post to /albums with Album "EminemAlbum1" and Song with id 1 then Album "EminemAlbum1" is returned with id 1
        mockMvc.perform(post("/albums")
                        .content("""
                                {
                                  "title": "EminemAlbum1",
                                  "releaseDate": "2026-03-13T15:56:56.740Z",
                                  "songIds": [1]
                                }
                                """.trim())
                        .contentType(MediaType.APPLICATION_JSON_VALUE))
